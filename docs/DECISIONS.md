...
Decisión: distribuir Tipo de comida, Comodidades, Precio y Detalle en proporción a su contenido, sin forzar anchos idénticos ni concentrar el espacio libre en una sola zona; mantener precio en regular, usar una línea terracota gruesa a la izquierda de la fotografía en hover/foco y separar metadata con líneas verticales. Desde 1180 px, la retícula usa `minmax(250px, 1.45fr) minmax(160px, 1fr) minmax(170px, 1.05fr) minmax(92px, 0.58fr) minmax(88px, 0.54fr)`: Comodidades conserva una proporción ligeramente mayor que Tipo, mientras Precio y Detalle crecen solo lo necesario para equilibrar sus márgenes internos. Entre 1179 y 521 px, la segunda fila usa `minmax(135px, 1fr) minmax(145px, 1.05fr) minmax(78px, 0.62fr) minmax(76px, 0.58fr)`; a 520 px o menos la acción conserva su fila propia y Precio mantiene un mínimo de 82 px para no recortar `Económico`. En `restaurant-main`, mostrar ubicación arriba, insignia editorial cuando corresponda y nombre después, seguido del horario, sin repetir ubicación; esa copia conserva su eje izquierdo y el flujo natural desde arriba. Las insignias Top y Destacada se sitúan antes del nombre dentro de un wrapper común, con `gap` estable y sin alterar el tratamiento anti-huérfanas. `restaurant-cuisine`, `restaurant-amenities` y `restaurant-price` alinean sus grupos hacia el borde izquierdo de cada columna para formar un sistema común y evitar una curva visual según la longitud del contenido. La fila usa `padding-block` simétrico; el área de comodidades mantiene su posición aun sin ítems y muestra `No informado` alineado a la izquierda. Tipo de comida y Comodidades muestran como máximo sus tres primeros elementos en la fila, sin modificar los arreglos fuente ni añadir un contador oculto.
...
Reemplaza: carrusel editorial por zonas norte, centro y sur, su ubicación posterior al directorio y la especificación histórica de navegación sin botones anterior/siguiente.
...
Presentación del home: mientras la consulta normalizada contiene texto, ocultar completamente el header editorial mediante su estado `hidden`; el navbar y el directorio permanecen visibles y el autoplay se detiene. Al vaciar la consulta —incluidos valores compuestos solo por espacios o una acción de reset que borra el buscador— restaurar el header y su autoplay. Los filtros por sí solos no cambian esta visibilidad.
...
Decisión actualizada el 12.09.2026: evolucionar el modal nativo actual, sin crear páginas ni perder cierre, Escape, foco devuelto o navegación circular sobre el conjunto filtrado. Mantener una franja superior independiente con `Ficha X de N` y cierre, y el footer anterior/siguiente fuera del único scroll interno. El hero conserva insignia editorial, localidad/región y nombre, con la escala contenida `clamp(2.35rem, 4.8vw, 4.35rem)` y los ajustes móviles y anti-huérfanas existentes.

Layout vigente: Sobre esta cocinería a ancho completo; franja de Platos destacados / Tipo de comida / Rango de precio; bloque central con mapa a la izquierda y dirección sobre información práctica a la derecha; Reseñas en Google a ancho completo; Información y contacto al final, incluyendo Sobre los datos. Información práctica agrupa Horario, Comodidades y Capacidad / Eventos / Catering. Hasta 980 px el centro se apila dirección → mapa → información práctica; hasta 720 px se apilan resumen gastronómico y contacto; hasta 520 px se apilan los tres servicios. La referencia define jerarquía y posiciones, no nuevos contenidos ni una copia pixel-perfect.

Reemplazado: el orden anterior de introducción/retícula práctica y el experimento de contenido completo a la izquierda con mapa aislado a la derecha. Sobre los datos deja de ser una sección final independiente y se integra al bloque de contacto sin perder información.

Trazabilidad: una imagen cuyo `imageKind` no sea `direct` se identifica como referencia territorial. Se preserva la descripción demo actual y su advertencia explícita; los platos proceden exclusivamente de `specialties`. No se copian valores de la captura. Top N y Destacadas mantienen sus fuentes editoriales existentes, sin listas paralelas. Los servicios simples ya comunicados en Capacidad y servicios no se duplican en planificación; las cláusulas con matices se conservan.

Mapas: se conserva el placeholder simulado, identificado como demostración y sin coordenadas reales. No se reactiva OpenStreetMap ni se integra Google. El criterio anterior de preview diferido con coordenadas validadas y atribución queda como antecedente de una implementación inactiva, no como descripción del mapa visible.
