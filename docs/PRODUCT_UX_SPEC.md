...
- El buscador usa `Nombre, lugar, comida...` y se alinea con `results-area`; su nombre accesible explicita nombre, ubicación, comida, comodidades y precio.
- La escritura libre busca sobre un índice común de nombre canónico/presentado, nombre alternativo, región, provincia, comuna, localidad, dirección, recinto, cocina, especialidades, descripción, servicios, categorías culinarias, comodidades visibles y precio real o presentado. El matching ignora mayúsculas, tildes, espacios exteriores y secuencias repetidas de espacios.
...
- Arquitectura vigente desde el 12.09.2026, basada en la referencia visual de layout: franja superior independiente con `Ficha X de N` y cierre; hero; Sobre esta cocinería; resumen gastronómico; ubicación e información práctica; Reseñas en Google; Información y contacto. El contador y la navegación usan el conjunto aplicable, nunca un total fijo.
...
- Ubicación conserva el SVG de mapa simulado y el aviso visible `Vista previa del mapa — demostración`, con texto alternativo que aclara que no hay coordenadas reales. No es una integración cartográfica definitiva. La lógica OpenStreetMap anterior permanece inactiva; reactivarla no forma parte de este cambio de layout.
...
- El bloque final se diferencia con el tratamiento vigente de `--paper` y organiza Contacto y Para planificar la visita en dos columnas, apiladas hasta 720 px. Planificación se omite si no aporta datos; las menciones simples de eventos/banquetería ya confirmadas en Capacidad y servicios no se repiten aquí. Se conservan las cláusulas calificadas o no confirmadas.
- Dentro de Información y contacto, `Sobre los datos` conserva estado, confianza, ID, clasificación, notas, fecha y enlaces de fuentes disponibles. La navegación anterior/siguiente permanece fuera del scroll, en el footer del diálogo.
