...
### Cierre de recuperación de la ficha — 13 de septiembre de 2026

- Reconstrucción: `911e9c5` contiene la reorganización original; `d71d622` contiene la separación de estados prácticos, la retirada del CSS de la introducción reemplazada y los cachebusters `20260912-2`. Al retomar, `main` coincidía con la referencia local `origin/main`, sin archivos nuevos ni cambios staged/unstaged. Esta continuación solo completa documentación; no modifica de nuevo la interfaz.
- Referencia original `Frame 1 (6).png` recuperada del adjunto de la primera sesión e inspeccionada realmente. La comparación confirma franja superior, hero, descripción, resumen gastronómico, mapa/práctica, reseñas y contacto. Se preservan las diferencias autorizadas: datos sin duplicar, mapa identificado como demostración y estados de servicios separados.
- La auditoría acumulada comprobó Top 1/2/3, Na Que Ver como Destacada, Ckunza Tilar sin platos/dirección/contacto, La Trocha con seis platos, Bruno Dorador con nombre largo y banquetería no confirmada, y los dos servicios afirmativos de Tierra Nuestra y Doña Paula. Diez casos aislados de funciones comprobaron también negativas explícitas, ausencia e incertidumbre, sin añadir registros ni modificar el dataset.
- Mediciones de la recuperación en 1440, 1180, 981, 980, 768, 721, 720, 521, 520, 420, 390 y 320 px: cero overflow horizontal del diálogo y su contenido. La continuación del 13.09 revalidó visualmente desktop 1440 × 1000, tablet 768 × 1024 y mobile 390 × 844, incluyendo grids, hero, mapa, contacto y estados vacíos/inciertos. El overflow externo de 11 px del navbar a 721 px también se reprodujo en una copia del commit `911e9c5`; queda fuera del modal y registrado en UX-003.
- Apertura con Enter/Espacio, cierre visible/Escape, foco devuelto, outline visible, navegación mediante botones y flechas, y reinicio de scroll comprobados. Con `mariscos` + Los Lagos se verificó `1 → 2 → 3 → 4 → 1` y `1 → 4`; una ficha única deshabilita ambos controles. En mobile se alcanzó el final del scroll (2079.5 de 2080 px, redondeo subpíxel), con cierre accesible y posición del fondo estable.
- Sin IDs duplicados ni referencias ARIA rotas en la ficha examinada; imágenes locales cargadas y alt contextual conservado. Sin errores ni advertencias en la consola capturada. El servidor registró el 404 preexistente de la solicitud automática a `favicon.ico`, ajeno al modal; HTML, CSS, JavaScript e imágenes solicitadas respondieron 200. No se encontraron restos nuevos de la migración; los helpers de descripción factual y OpenStreetMap siguen inactivos deliberadamente, según la arquitectura documentada.
- Accesibilidad básica: controles semánticos y recorrido de contactos/fuentes/navegación preservados. Los colores de texto informativo y foco comprobados superan 5:1 sobre las superficies claras del modal. El diálogo no tiene animación ni transición; las reglas CSS de reduced motion se inspeccionaron en navegador y el guard de autoplay se comprobó con simulación Node. No se realizó emulación de esa preferencia en navegador ni una certificación WCAG integral; QA-001 sigue abierto.
- Sintaxis de `data.js`, `script.js` y los tres scripts `.mjs`, más `git diff --check`, reejecutados para el cierre. `PRODUCT_UX_SPEC`, DEC-019 y arquitectura ya estaban sincronizados; esta continuación completa estado, backlog y changelog sin introducir decisiones de diseño posteriores.
